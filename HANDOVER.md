...
Status on 2026-09-19: milestones 1 to 3 are built and verified, milestone 4 (tailor) is built and verified on a live run; see section 12. This document is the specification. Read it fully, then `RESEARCH.md` for the evidence behind each decision, then the design files in `design/`. Do not re-open decisions marked locked unless the owner asks.
...
2. ~~**Profile bootstrap.**~~ **Done.** `claude.js` (shared API layer), `prompts.js`, `tools/run.mjs` (dev harness with a spend ledger). Verified on the owner's real resume: 15 bullets extracted word for word, 12 numbers captured with what each measures, 11 parse-review items including both US spellings, soft hyphens, a missing GitHub URL and a missing separator in a skills label.
3. ~~**Job workspace.**~~ **Done.** Analysis, weighted requirements, role-shape detection, predicted skills, gap questions, answers becoming profile bullets. Verified on all four real Melbourne ads: the MLOps ad is correctly detected as MLE rather than DA, and the Kepler "Data Analyst" ad is correctly called out as data operations rather than insight work.

**Verified while building 2 and 3, do not re-learn the hard way:**
- **Always stream.** These calls run 30 to 90 seconds. A non-streaming request that long has its socket closed underneath it (`UND_ERR_SOCKET` in node, a stalled request in the browser). `claude.js` streams every call and reassembles the message, so callers never see the difference.
- **Structured output schemas reject `minimum`/`maximum` on integers** and are picky about extra keywords. Put ranges in the description and restate them in the prompt.
- **Model choice: use Opus 5 for both calls.** Sonnet 5 was measured against it on the same inputs. On extraction it found 5 parse-review items against Opus's 11, dropped a number, converted the resume's curly quotes to straight ones, and took three times as long. On analysis it found 18 requirements against Opus's 24. It is barely cheaper because it emits roughly twice the output tokens, so the saving is a few cents while the quality drop is real.
- **Extraction keeps spelling verbatim** and raises US spellings in the parse review instead of silently changing them. This deviates from the original spec line "spelling to en-AU", and deliberately: silently rewording someone's own resume breaks the truth-first rule that the whole tool exists to enforce.
- **Prompts live in `prompts.js`, not `prompts/*.txt`.** A `fetch` of a local text file fails under `file://`, and double-click has to keep working.
- **Measured cost per run on Opus 5 at high effort:** extraction about $0.11, ad analysis about $0.17, tailor about $0.19.
4. ~~**Tailor.**~~ **Done.** Live run on the Junior Data Analyst ad: 65s, $0.19 on Opus 5 at high effort, 14 changes, every one with sources, no unsourced number. Design refinement while building: the tailored document is never stored. `job.tailored` holds `version` and a `changes` list only, and `RT.buildDoc(profile, tailored, settings)` rebuilds the page on every render. The model returns changes, the app fills `original` from the profile, and a target with an accepted change is skipped on a re-tailor (`RT.normaliseChanges`), so accepted wording survives by construction. The person's own edits, hides and moves are changes too, with `sources: [{type: "user"}]`, and go through the same numbers check. Verified in the browser on the real profile: page renders at A4 and scales to the panel, page fit measures the real layout (107% before tailoring, trim button appears past the target), keyword coverage and the cv column, lint badges and list, select/edit/hide/move/undo, A/R/E/J/K keys. The document is paginated in the browser (`paginate` in app.js): flow nodes are placed on 210 x 297mm sheets and moved to the next sheet when they overflow, with headings and item heads kept with what follows. Each sheet carries its own margins as padding, so `@page` margin stays zero at any page count and the browser adds no header or footer. The Profile inventory uses the same sheets. Not yet verified: the print output itself (needs a manual Save as PDF pass). The "AI-rewrite with a note" and "insert an unused bullet" block actions from section 9 are not built; the model's `add-from-inventory` covers the second.
